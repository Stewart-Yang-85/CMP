...
- **适用范围**：SIM Dependent Bundle / Fixed Bundle / Tiered Pricing（monthly recurring）
...
- 资费类型（四选一）：
  ...
  - Tiered Pricing（monthly recurring，阶梯计费）
...
##### Tiered Pricing（阶梯计费，monthly recurring）规则（结论）
...
- [x] 阶梯计费口径：Tiered Pricing 采用分段累进（Progressive），不采用全量按档（All-in）
