import { createCarrierService, createCommercialTerms, createControlPolicy, cloneCommercialTerms, cloneControlPolicy, deprecateCarrierService, deprecateCommercialTerms, deprecateControlPolicy, getCarrierServiceDetail, getCommercialTermsDetail, getControlPolicyDetail, listCarrierServices, listCommercialTerms, listControlPolicies, publishCarrierService, publishCommercialTerms, publishControlPolicy, updateCarrierService, updateCommercialTerms, updateControlPolicy, validateCarrierServiceModule, validateCommercialTermsModule, validateControlPolicyModule, formatCarrierServiceValidationResponseForApi, } from '../services/package.js';
export function registerPackageModuleRoutes({ app, prefix, deps }) {
    ...
    function carrierAuthResellerTenantId(req, res, auth) {
    ...
    app.post(`${prefix}/commercial-terms`, async (req, res) => {
        ...
        if (authResellerTenantId === undefined)
            return;
    ...
    app.post(`${prefix}/commercial-terms/:commercialTermsId\\:publish`, async (req, res) => {
        const auth = ensureResellerAdmin(req, res);
        if (!auth)
            return;
        const authResellerTenantId = carrierAuthResellerTenantId(req, res, auth);
    ...
    app.post(`${prefix}/commercial-terms/:commercialTermsId\\:deprecate`, async (req, res) => {
        const auth = ensureResellerAdmin(req, res);
        if (!auth)
            return;
        const authResellerTenantId = carrierAuthResellerTenantId(req, res, auth);
    ...
    app.post(`${prefix}/control-policies/:controlPolicyId\\:publish`, async (req, res) => {
        const auth = ensureResellerAdmin(req, res);
        if (!auth)
            return;
        const authResellerTenantId = carrierAuthResellerTenantId(req, res, auth);
    ...
    app.post(`${prefix}/control-policies/:controlPolicyId\\:deprecate`, async (req, res) => {
        const auth = ensureResellerAdmin(req, res);
        if (!auth)
            return;
        const authResellerTenantId = carrierAuthResellerTenantId(req, res, auth);
    ...
    app.get(`${prefix}/control-policies`, async (req, res) => {
        ...
        if (!auth)
            return;
        const qCp = req.query ?? {};
        const { status, page, pageSize, } = qCp;
        const resellerIdRawCp = qCp.resellerId ?? qCp.reseller_id;
        const resellerIdQueryCp = resellerIdRawCp !== undefined && resellerIdRawCp !== null && String(resellerIdRawCp).trim() !== ''
        ...
        if (auth.scope === 'reseller') {
            ...
            if (authResellerListCp === undefined)
                return;
    ...
    app.post(`${prefix}/carrier-services`, async (req, res) => {
        ...
        const result = await createCarrierService({
            ...
            auth: {
                scope: auth.scope,
                resellerTenantId: req.cmpAuth?.resellerId ?? null,
            },
        });
        if (!result.ok)
            return sendError(res, result.status, result.code, result.message);
    ...
    app.post(`${prefix}/carrier-services/:carrierServiceId\\:publish`, async (req, res) => {
        const auth = ensureResellerAdmin(req, res);
        if (!auth)
            return;
        const authResellerTenantId = carrierAuthResellerTenantId(req, res, auth);
    ...
    app.post(`${prefix}/carrier-services/:carrierServiceId\\:deprecate`, async (req, res) => {
        const auth = ensureResellerAdmin(req, res);
        if (!auth)
            return;
        const authResellerTenantId = carrierAuthResellerTenantId(req, res, auth);
    ...
    app.get(`${prefix}/carrier-services`, async (req, res) => {
        ...
        if (!auth)
            return;
        const { apnProfileId, roamingProfileId, status, page, pageSize, resellerId: resellerIdQueryRaw, supplierId: supplierIdRaw, operatorId: operatorIdRaw, carrierId: carrierIdRaw, enterpriseId: enterpriseIdQueryRaw, } = req.query ?? {};
        if (enterpriseIdQueryRaw !== undefined &&
            enterpriseIdQueryRaw !== null &&
            String(enterpriseIdQueryRaw).trim() !== '') {
            return sendError(res, 400, 'BAD_REQUEST', 'enterpriseId is not supported for carrier service list; catalog modules are reseller-scoped (enterprise is expressed on Package / Price Plan).');
        }
        const operatorIdQuery = operatorIdRaw ?? carrierIdRaw ?? null;
        const supabase = createSupabaseRestClient({ useServiceRole: true, traceId: getTraceId(res) });
        const resellerIdQuery = resellerIdQueryRaw !== undefined && resellerIdQueryRaw !== null && String(resellerIdQueryRaw).trim() !== ''
