...
export function registerReconciliationRoutes({ app, prefix, deps }) {
  const { createSupabaseRestClient, getTraceId, sendError, ensurePlatformAdmin, isValidUuid } = deps

  app.get(`${prefix}/reconciliation/runs`, async (req, res) => {
  ...
  app.post(`${prefix}/reconciliation/runs`, async (req, res) => {
    const auth = ensurePlatformAdmin(req, res)
    if (!auth) return
    const body = req.body ?? {}
  ...
  app.get(`${prefix}/reconciliation/runs/:runId/mismatches`, async (req, res) => {
    const auth = ensurePlatformAdmin(req, res)
    if (!auth) return
  ...
  app.get(`${prefix}/reconciliation/runs/:runId/mismatches/:iccid/trace`, async (req, res) => {
    const auth = ensurePlatformAdmin(req, res)
    if (!auth) return
    const runId = req.params?.runId ? String(req.params.runId).trim() : ''
    if (!runId || !isValidUuid(runId)) {
  ...
  app.get(`${prefix}/reconciliation/runs/:runId`, async (req, res) => {
    const auth = ensurePlatformAdmin(req, res)
    if (!auth) return
    const runId = req.params?.runId ? String(req.params.runId).trim() : ''
    if (!runId || !isValidUuid(runId)) {
