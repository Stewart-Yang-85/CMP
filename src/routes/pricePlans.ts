...
export function registerPricePlanRoutes({ app, prefix, deps }: { app: any; prefix: string; deps: Deps }) {
   ...
   */
  function resolveResellerIdForPricePlanRoute(
    req: any,
    res: any,
  ...
  async function assertPricePlanResellerScope(req: any, res: any, supabase: any, auth: { scope?: string | null }, pricePlanId: string) {
    ...
    const tokenR = String(req?.cmpAuth?.resellerId ?? '').trim()
    const rowR = String((plan as any).reseller_id ?? '').trim()
    if (rowR && tokenR && rowR === tokenR) return true
    if (rowR && tokenR && rowR !== tokenR) {
      sendError(res, 403, 'FORBIDDEN', 'Price plan is out of reseller scope.')
      return false
    }
    const enterpriseId = await resolveEnterpriseForReseller(req, res, supabase, String((plan as any).enterprise_id))
    return !!enterpriseId
  }
  ...
  app.put(`${prefix}/price-plans/:pricePlanId`, async (req: any, res: any) => {
    const auth = ensureResellerAdmin(req, res)
    if (!auth) return
    const pricePlanId = String(req.params.pricePlanId || '').trim()
    if (!isValidUuid(pricePlanId)) {
  ...
  app.post(`${prefix}/price-plans/:pricePlanId/publish`, async (req: any, res: any) => {
    const auth = ensureResellerAdmin(req, res)
    if (!auth) return
    const pricePlanId = String(req.params.pricePlanId || '').trim()
    if (!isValidUuid(pricePlanId)) {
  ...
  app.post(`${prefix}/price-plans/:pricePlanId/deprecate`, async (req: any, res: any) => {
    const auth = ensureResellerAdmin(req, res)
    if (!auth) return
    const pricePlanId = String(req.params.pricePlanId || '').trim()
    if (!isValidUuid(pricePlanId)) {
