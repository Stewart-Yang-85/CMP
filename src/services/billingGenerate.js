...
async function loadPackageVersions(supabase, packageVersionIds) {
...
export async function runBillingGenerate({
  ...
  for (const enterprise of enterprises) {
    ...
    const packageIds = Array.from(new Set(calc.lineItems.map((i) => i.package_version_id).filter(Boolean).map(String)))
