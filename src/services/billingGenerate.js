...
async function loadEnterpriseList(supabase, enterpriseId, resellerId) {
    if (enterpriseId) {
        const rows = await supabase.select('tenants', `select=tenant_id,parent_id,name&tenant_id=eq.${encodeURIComponent(enterpriseId)}&tenant_type=eq.ENTERPRISE&limit=1`);
        const row = Array.isArray(rows) ? rows[0] : null;
        if (!row)
            return [];
        if (resellerId && String(row.parent_id || '') !== String(resellerId))
            return [];
        return [row];
    }
    if (resellerId) {
        const rows = await supabase.select('tenants', `select=tenant_id,parent_id,name&parent_id=eq.${encodeURIComponent(resellerId)}&tenant_type=eq.ENTERPRISE`);
        return Array.isArray(rows) ? rows : [];
    }
    const rows = await supabase.select('tenants', 'select=tenant_id,parent_id,name&tenant_type=eq.ENTERPRISE');
    return Array.isArray(rows) ? rows : [];
}
function aggregateLineItems({ lineItems, sims, packages, departments, }) {
    ...
    for (const pkg of packages) {
        ...
        if (pid)
            packageMeta.set(pid, pkg);
    }
    const departmentMeta = new Map();
    for (const dept of departments) {
        if (dept?.tenant_id)
            departmentMeta.set(String(dept.tenant_id), dept);
    }
    const extraItems = [];
    for (const item of lineItems) {
        ...
        if (item.item_type === 'MONTHLY_FEE') {
            ...
            if (pkgId)
                current.packageVersionId = pkgId;
        }
...
async function loadPackagesForBilling(supabase, packageIds) {
    if (!packageIds.length)
        return [];
    const idFilter = packageIds.map((id) => encodeURIComponent(id)).join(',');
    const rows = await supabase.select('packages', `select=package_id,name&package_id=in.(${idFilter})`);
    return Array.isArray(rows) ? rows : [];
}
async function loadDepartments(supabase, departmentIds) {
    if (!departmentIds.length)
        return [];
    const idFilter = departmentIds.map((id) => encodeURIComponent(id)).join(',');
    const rows = await supabase.select('tenants', `select=tenant_id,name&tenant_id=in.(${idFilter})&tenant_type=eq.DEPARTMENT`);
    return Array.isArray(rows) ? rows : [];
}
export async function runBillingGenerate({ supabase, period, enterpriseId, resellerId, autoPublish, actorUserId, actorRole, requestId, sourceIp, jobId, }) {
    ...
    for (const enterprise of enterprises) {
        ...
        if (!calc)
            continue;
        const existing = await supabase.select('bills', `select=bill_id&enterprise_id=eq.${encodeURIComponent(enterprise.tenant_id)}&period_start=eq.${encodeURIComponent(toDateOnly(start))}&period_end=eq.${encodeURIComponent(toDateOnly(endInclusive))}&limit=1`);
        if (Array.isArray(existing) && existing.length) {
            continue;
        }
        const simsRows = await supabase.select('sims', `select=sim_id,iccid,msisdn,department_id&enterprise_id=eq.${encodeURIComponent(enterprise.tenant_id)}`);
        const sims = Array.isArray(simsRows) ? simsRows : [];
        const departmentIds = Array.from(new Set(sims.map((s) => s.department_id).filter(Boolean).map(String)));
        const packageIds = Array.from(new Set(calc.lineItems
            .map((i) => i.package_id ?? i.package_version_id)
            .filter(Boolean)
            .map(String)));
