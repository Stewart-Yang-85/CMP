...
async function resolveDunningPolicy(supabase, enterpriseId, parentId) {
...
async function resolveLateFeeRule(supabase, enterpriseId, parentId) {
...
async function listOverdueBills(supabase, enterpriseId, asOfDate) {
  const filters = [
...
export async function getEnterpriseDunningSummary({
  ...
  const daysOverdue = oldestDue ? Math.max(0, diffDays(oldestDue, parseDateOnly(today))) : 0
...
export async function runDunningCheck({
  ...
  for (const [eid, bills] of byEnterprise.entries()) {
    ...
    for (const bill of bills) {
      ...
      const daysOverdue = dueDate ? Math.max(0, diffDays(dueDate, parseDateOnly(today))) : 0
      ...
      if (!existing) {
        ...
        const inserted = Array.isArray(insertRows) ? insertRows[0] : null
