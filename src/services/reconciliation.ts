...
function buildMismatchMetrics(items: Array<Record<string, unknown>>) {
  const byField: Record<string, number> = {}
  const byResolution: Record<string, number> = {}
  const byLocalStatus: Record<string, number> = {}
  const byUpstreamStatus: Record<string, number> = {}
  const byStatusPair: Record<string, number> = {}
  const byEnterpriseId: Record<string, number> = {}
  const bySupplierId: Record<string, number> = {}
  const byCarrierId: Record<string, number> = {}
...
async function loadRunRow(supabase: SupabaseClient, runId: string) {
...
function normalizeUpstreamSimStatus(value: unknown) {
...
async function updateSimFromUpstream({
  supabase,
  sim,
  upstreamStatus,
  traceId,
...
export async function createReconciliationRun({
  ...
  scope?: string | null
  traceId?: string | null
}): Promise<ServiceResult<{ runId: string; supplierId: string; status: string; date: string; scope: string; traceId: string | null }>> {
  ...
  const run = Array.isArray(rows) ? (rows[0] as Record<string, any>) : null
...
export async function runReconciliation({
  ...
  date: string
  status: string
  summary: { totalSimsChecked: number; matched: number; mismatched: number; localOnly: number; upstreamOnly: number }
  mismatches: Array<Record<string, unknown>>
  metrics: ReturnType<typeof buildMismatchMetrics>
}>> {
  if (!supabase) return toError(500, 'INTERNAL_ERROR', 'supabase client is required.')
  const scopeValue = normalizeScope(scope)
  ...
  for (const sim of list) {
    const localStatus = sim?.status ? String((sim as Record<string, unknown>).status) : ''
    const upstreamStatus = sim?.upstream_status ? String((sim as Record<string, unknown>).upstream_status) : ''
    if (!upstreamStatus) {
      localOnly += 1
      continue
    }
    ...
    mismatchDetails.push({
      iccid: (sim as Record<string, any>).iccid,
      simId: (sim as Record<string, any>).sim_id ?? null,
      enterpriseId: (sim as Record<string, any>).enterprise_id ?? null,
      supplierId: (sim as Record<string, any>).supplier_id ?? null,
      operatorId: (sim as Record<string, any>).operator_id ?? null,
      carrierId: (sim as Record<string, any>).operator_id ?? null,
      field: 'status',
      localValue: localStatus,
      upstreamValue: upstreamStatus,
      upstreamStatusUpdatedAt: (sim as Record<string, any>).upstream_status_updated_at ?? null,
...
export async function listReconciliationMismatches({
...
}): Promise<ServiceResult<{ items: Array<Record<string, unknown>>; total: number; page: number; pageSize: number; metrics: ReturnType<typeof buildMismatchMetrics> }>> {
  if (!supabase) return toError(500, 'INTERNAL_ERROR', 'supabase client is required.')
  if (!runId || !isValidUuid(runId)) {
    return toError(400, 'BAD_REQUEST', 'runId must be a valid uuid.')
  }
  ...
  const run = Array.isArray(rows) ? (rows[0] as Record<string, any>) : null
  if (!run?.run_id) return toError(404, 'NOT_FOUND', 'reconciliation run not found.')
  let items: Array<Record<string, unknown>> = Array.isArray(run.mismatch_details) ? run.mismatch_details : []
  ...
  if (fieldValue) {
    items = items.filter((it) => String((it as Record<string, unknown>)?.field ?? '') === fieldValue)
  }
  if (resolutionValue) {
    items = items.filter((it) => String((it as Record<string, unknown>)?.resolution ?? '') === resolutionValue)
  }
  if (iccidValue) {
    items = items.filter((it) => String((it as Record<string, unknown>)?.iccid ?? '') === iccidValue)
  }
  if (enterpriseValue) {
    items = items.filter((it) => String((it as Record<string, unknown>)?.enterpriseId ?? '') === enterpriseValue)
  }
...
export async function getReconciliationMismatchTrace({
  supabase,
  runId,
  iccid,
}: {
  ...
  const run = Array.isArray(runRows) ? (runRows[0] as Record<string, any>) : null
  if (!run?.run_id) return toError(404, 'NOT_FOUND', 'reconciliation run not found.')
  const mismatchDetails = Array.isArray(run.mismatch_details) ? run.mismatch_details : []
  const mismatch = mismatchDetails.find((it: Record<string, unknown>) => String(it?.iccid ?? '') === iccidValue) ?? null
  ...
  const sim = Array.isArray(simRows) ? (simRows[0] as Record<string, any>) : null
  const simId = sim?.sim_id ? String(sim.sim_id) : null
  let simStateHistory: Array<Record<string, unknown>> = []
  if (simId) {
    ...
    simStateHistory = Array.isArray(historyRows) ? historyRows.map((r: any) => ({
  ...
  const audits = Array.isArray(auditRows) ? auditRows.map((r: any) => ({
    auditId: r.audit_id,
    actorUserId: r.actor_user_id ?? null,
