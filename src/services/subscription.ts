...
async function loadPackageVersion(supabase: SupabaseClient, packageVersionId: string) {
  const rows = await supabase.select(
    'package_versions',
    `select=package_version_id,package_id,status,commercial_terms,price_plan_id,effective_from&package_version_id=eq.${encodeURIComponent(packageVersionId)}&limit=1`
  )
  return Array.isArray(rows) ? (rows[0] as Record<string, unknown>) : null
}
...
async function loadPricePlanVersion(supabase: SupabaseClient, pricePlanId: string) {
  const rows = await supabase.select(
    'price_plans',
    `select=price_plan_id,type,validity_days,payg_rates&price_plan_id=eq.${encodeURIComponent(pricePlanId)}&limit=1`
  )
...
function resolveExpiryBoundary(terms: ReturnType<typeof normalizeCommercialTerms>, paygRates: unknown) {
  const meta = paygRates && typeof paygRates === 'object' ? (paygRates as Record<string, unknown>).meta : undefined
  const fromMeta = meta && typeof meta === 'object' ? (meta as Record<string, unknown>).expiryBoundary : undefined
...
export async function createSubscription({
  ...
  tenantFilter: string
  audit?: AuditContext
}): Promise<ServiceResult<{ subscriptionId: string | null; state: string; effectiveAt: string; expiresAt: string | null; commitmentEndAt: string | null }>> {
...
export async function switchSubscription({
  ...
  tenantFilter: string
  audit?: AuditContext
}): Promise<ServiceResult<{ cancelledSubscriptionId: string | null; newSubscriptionId: string | null; effectiveAt: string }>> {
  if (!isValidUuid(enterpriseId)) {
    return toError(400, 'BAD_REQUEST', 'enterpriseId must be a valid uuid.')
  }
  const iccidValue = String(iccid || '').trim()
...
export async function cancelSubscription({
  ...
  immediate?: unknown
  audit?: AuditContext
}): Promise<ServiceResult<{ subscriptionId: string; state: string; expiresAt: string }>> {
  if (!isValidUuid(enterpriseId)) {
    return toError(400, 'BAD_REQUEST', 'enterpriseId must be a valid uuid.')
  }
  ...
  if (packageVersionIds.length) {
    ...
    if (Array.isArray(versions)) {
      for (const v of versions) {
        ...
        if (row.package_version_id) versionMap.set(String(row.package_version_id), row)
      }
    }
  }
