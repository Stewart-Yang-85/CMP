...
export async function listApnProfiles({ supabase, supplierId, operatorId, apnProfileId, status, page, pageSize }) {
  const filters = []
...
export async function listRoamingProfiles({ supabase, supplierId, operatorId, roamingProfileId, status, page, pageSize }) {
  const filters = []
  const supplierIdValue = supplierId ? String(supplierId) : null
  const operatorIdValue = operatorId ? String(operatorId) : null
...
export async function cloneApnProfile({ supabase, apnProfileId, payload, audit }) {
  ...
  const config = latestVersion?.config ?? {
    apn: source.apn,
    authType: source.auth_type,
    username: source.username ?? null,
    passwordRef: source.password_ref ?? null,
  }
  const versionRows = await insertProfileVersionWithFallback(
    supabase,
    {
...
export async function cloneRoamingProfile({ supabase, roamingProfileId, payload, audit }) {
  ...
  const config = latestVersion?.config ?? { mccmncList: source.mccmnc_list }
  const versionRows = await insertProfileVersionWithFallback(
    supabase,
    {
      profile_type: 'ROAMING',
...
export async function updateApnProfile({ supabase, apnProfileId, payload, audit }) {
  ...
  if (latestVersion?.profile_version_id) {
    const config = {
      apn: updates.apn ?? profile.apn,
      authType: updates.auth_type ?? profile.auth_type,
