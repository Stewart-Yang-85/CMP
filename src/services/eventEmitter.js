...
async function enrichPayloadWithResellerId(supabase, tenantId, payload) {
    const resellerTenantId = await resolveResellerTenantIdFromContext(supabase, tenantId ?? null);
    if (!resellerTenantId)
        return payload;
    return { ...payload, resellerId: resellerTenantId };
}
...
async function isDuplicateEvent({ supabase, eventType, tenantId, payload, occurredAt, }) {
    ...
    if (normalizedTenantId)
        baseFilters.push(`tenant_id=eq.${encodeURIComponent(normalizedTenantId)}`);
    ...
    if (eventType === 'SIM_STATUS_CHANGED') {
        ...
        if (!afterStatus || !simId || !normalizedTenantId)
            return false;
        return rows.some((row) => {
            const rowPayload = normalizePayload(row.payload);
            return (normalizeValue(rowPayload.afterStatus ?? rowPayload.after_status) === afterStatus &&
    ...
    if (eventType === 'SUBSCRIPTION_CHANGED') {
        ...
        if (!subscriptionId || !afterState || !effectiveAt || !normalizedTenantId)
            return false;
        return rows.some((row) => {
            const rowPayload = normalizePayload(row.payload);
            return (normalizeValue(rowPayload.subscriptionId) === subscriptionId &&
    ...
    if (eventType === 'SUBSCRIPTION_PROVISION_FAILED') {
        ...
        if (!subscriptionId || !errorCode || !normalizedTenantId)
            return false;
        return rows.some((row) => {
            const rowPayload = normalizePayload(row.payload);
            return (normalizeValue(rowPayload.subscriptionId) === subscriptionId &&
    ...
    if (eventType === 'PAYMENT_CONFIRMED') {
        ...
        if (!billId || !customerId || !paymentRef)
            return false;
        return rows.some((row) => {
            const rowPayload = normalizePayload(row.payload);
            return (normalizeValue(rowPayload.billId) === billId &&
...
export async function emitEvent(input) {
    ...
    let payload = normalizePayload(input.payload ?? null);
    payload = await enrichPayloadWithResellerId(supabase, input.tenantId ?? null, payload);
    ...
    if (duplicate) {
        return { duplicate: true, eventId: null, webhookDeliveryIds: [] };
    }
    ...
    if (event?.event_id) {
        if (input.dispatchWebhooks === false) {
            return { duplicate: false, eventId: event.event_id, webhookDeliveryIds: [] };
        }
        try {
            ...
            return {
                ...
                webhookDeliveryIds: Array.isArray(webhookResult?.deliveryIds)
                    ? webhookResult.deliveryIds.map((id) => Number(id)).filter((id) => Number.isFinite(id))
                    : [],
            };
        }
        catch {
    ...
    return { duplicate: false, eventId: null, webhookDeliveryIds: [] };
}
