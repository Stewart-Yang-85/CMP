...
type SupabaseClient = {
  select: (table: string, query: string, options?: { suppressMissingColumns?: boolean }) => Promise<unknown>
  update: (table: string, match: string, patch: unknown, options?: { returning?: 'minimal' | 'representation' }) => Promise<unknown>
}
...
function toCutoffIso(days: number, now: Date) {
  return new Date(now.getTime() - days * 24 * 60 * 60 * 1000).toISOString()
}

async function loadSuspendedEnterpriseIds({
  supabase,
  cutoffIso,
  limit,
  offset,
  ...
  return Array.isArray(rows) ? rows.map((row) => String((row as { tenant_id?: string }).tenant_id || '')).filter(Boolean) : []
}

async function anonymizeSuspendedEnterprises({
  supabase,
  cutoffIso,
  batchSize,
  nowIso,
...
export async function runGdprRetention(input: RetentionInput = {}) {
